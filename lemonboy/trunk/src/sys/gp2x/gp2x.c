...
void vid_begin(){
	...
	if(lastenabled){
		...
		case vmode_ipoldbl: // INTERPOLATION Double
			...
			// Rounding was done on the rows, extra rows where 
			...
			while(tempy--){
				if ( line % 30 != 0 )
				{
					MEMCPY(s1+17*fb.pelsize, fs, 286*fb.pelsize);
					s1+=fb.pitch;
				}
				fs+=320*fb.pelsize;
				line++;
		...
		case vmode_ipol: // INTERPOLATION
			...
			tempy=192;
			s0+=320*13*fb.pelsize;
			s1+=320*24*fb.pelsize;
			while(tempy--){
				MEMCPY(s1+53*fb.pelsize, s0, 214*fb.pelsize);
				s0+=320*fb.pelsize;
				s1+=fb.pitch;
			}			
			break;
...
#define RANGE(M,X,V) V=V<M?M:(V>X?X:V)
	...
	RANGE(0, MAX_VIDEO_MODES, options.scaling);
